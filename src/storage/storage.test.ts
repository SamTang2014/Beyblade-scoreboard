...
describe('直播設定', () => {
  ...
  /**
   * 入分 link 嗰部機由條 link 砌返個 `live`，而條 link 淨係帶住 edit token ——
   * 吉 view 唔可以當「冇分享」，唔係佢會靜靜雞連唔到，
   * 入分入到爽但一分都推唔上去。
   */
  it('吉 view 照收 —— 入分 link 嗰部機未必知', () => {
    const t = parseTournament({
      id: 't1', name: '賽事', createdAt: 0, updatedAt: 0,
      mode: 'roundRobin', players: [], matches: [],
      live: { scriptId: 'a', edit: 'b', view: '' },
    })
    expect(t.live).toEqual({ scriptId: 'a', edit: 'b', view: '' })
  })
