...
TopBar 出「入分位喺第二部機」。撳「收返入分位」就會**用佢部機嗰份蓋過線上嗰份**。

有一樣要講清楚：佢**本機存住嗰份唔會俾遠端蓋走**（poll 見到有嘢未推就唔會
adopt）—— 但**未推嘅改動係喺記憶體度**，reload 個 tab 就冇咗，嗰陣本機
就會跟返線上嗰份。即係「唔會靜靜雞冇咗」呢句，喺唔 reload 嘅前提下先成立。
...
逐場賽事喺 localStorage 記一個（`beyblade-scoreboard/v/<id>`）。

**唔記住嘅話有真 bug。** `version` 每次 mount 由 `null` 開始 = 「我乜都唔知」
= 第一次 poll 一定攞成份遠端返嚟然後 adopt，**唔理佢係咪比本機舊**。

實際出過事：

1. 主辦喺開賽設定揀「認真」→ 開始直播。**呢刻推上去嗰份 `matches: []`** ——
   因為「規模」呢個選擇喺「加人」同「排賽程」上面
2. 加人 → 排賽程 → 本機有咗成個賽程
3. 俾第二部機搶咗入分位 → 推唔上去
4. reload 一次 → `version` 歸零 → poll 攞返步驟 1 嗰個舊 snapshot
5. **成個賽程冇咗**，入分版變返「仲未排賽程。去加選手同排賽程」

記住之後，poll 用 `since=<記住嗰個>` 去問，遠端冇行前過就乜都唔會派落嚟。

三個地方要記：`init` 成功之後、每次 push 成功之後、每次 adopt 之後。
條 link 死咗（`bad-token`）就要 `forgetVersion` —— 嗰個號碼講緊另一場賽事。

**開入分 link 嗰陣都要守同一條線。** `Live.tsx` 存落本機之前要比較：
本機有嗰份、而且「我最後知嘅版本」>= 遠端而家嗰個，就保住本機，
淨係補返個 `live` 設定。
...
「**由你張 sheet 接返場賽事**」：貼 script 網址 + edit token → 拉返成場賽事。
兩個 token 就寫喺張 sheet B1／B2，主辦開自己個 Drive 就抄得返。

**擺位係關鍵。** 部機爆咗嗰陣，新部機**一場賽事都冇** —— 如果呢個入口埋喺
「某一場賽事嘅設定頁」入面，等於冇路入去：你要先開一場垃圾賽事、入設定、
揀認真，先撳到，而且救完仲會留低嗰場空賽事。

所以佢要出現喺**「乜都冇」嗰個畫面**度，即係主頁。收埋做一行細字（`<details>`），
唔會阻住正常開賽事嘅人。
